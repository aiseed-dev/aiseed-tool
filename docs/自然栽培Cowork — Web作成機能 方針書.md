...
```
cowork.aiseed.dev/username/
```

- 全ユーザーを1ドメインに集約（SEOのドメインパワーが集中）
- ユーザーが増えるほど全体の検索評価が上がる
- ユーザー側のドメイン設定不要

### PC版（独自ドメイン）

- 農家が自分のドメインを取得して運用
- cowork.aiseed.dev/username/ からリンクを設置（被リンク効果でSEOを支援）
- 独自ドメインを取っても cowork 側のページは残す
...
| AI生成（スマホ版） | Claude Agent SDK（Max定額プラン） |
...
|-----------|------|
| レート制限（429） | 待機して再試行 |
| SDK 一時エラー（タイムアウト等） | すぐ再試行 |
| データ不正（必須項目なし等） | 中止、ユーザーにメール通知 |
