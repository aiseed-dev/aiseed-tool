...
class OnboardingScreen extends StatefulWidget {
  ...
  final ValueChanged<String>? onSystemPromptGenerated;

  const OnboardingScreen({
    super.key,
    required this.settings,
    this.onSystemPromptGenerated,
  });
...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  bool get _canProceed {
    switch (_step) {
      case 0:
        return _selectedCrops.isNotEmpty || _cropsController.text.trim().isNotEmpty;
      case 1:
      ...
      case 4:
        return true; // Challenges is optional
      default:
        return false;
    }
  }
  ...
  void _generate() {
    // Collect custom crops from text field
    ...
    setState(() {
      ...
      _generatedSystemPrompt =
          SkillFileGenerator.generateSystemPrompt(profile);
      _step = _totalSteps; // Show result
    });
  }
    ...
    return Scaffold(
      ...
      body: Center(
        child: ConstrainedBox(
          ...
          child: _step < _totalSteps ? _buildQuestion() : _buildResult(),
        ),
      ),
    );
  }
          ...
          Row(
            ...
            children: [
              ...
              FilledButton.icon(
                ...
                icon: Icon(
                    _step == _totalSteps - 1 ? Icons.auto_awesome : Icons.arrow_forward),
                label: Text(_step == _totalSteps - 1 ? 'スキルファイルを生成' : '次へ'),
              ),
            ],
          ),
        ],
        ...
        Padding(
          ...
          child: Wrap(
            ...
            children: [
              ...
              OutlinedButton.icon(
                onPressed: _applyAsSystemPrompt,
                icon: const Icon(Icons.check_circle),
                label: const Text('Coworkに適用'),
              ),
            ],
          ),
        ),
  ...
  void _applyAsSystemPrompt() {
    if (_generatedSystemPrompt == null) return;
    widget.settings.setSystemPrompt(_generatedSystemPrompt!);
    widget.onSystemPromptGenerated?.call(_generatedSystemPrompt!);

    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Coworkのシステムプロンプトに適用しました'),
        duration: Duration(seconds: 2),
      ),
    );
